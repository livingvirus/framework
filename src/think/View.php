...
class View
{
    ...
    private function getTemplateTheme($module)
    {
        if ($this->config['theme_on']) {
            if ($this->theme) {
                ...
                if (!isset($theme) || !is_dir(APP_PATH . $module . $this->config['view_layer'] . DS . $theme)) {
    ...
    protected function getThemePath($module = MODULE_NAME)
    {
        ...
        if (!$tmplPath) {
            ...
            $tmplPath = defined('TMPL_PATH') ? TMPL_PATH . $module . DS : APP_PATH . $module . DS . $this->config['view_layer'] . DS;
        }
